...
2. 핵심 모듈
   ...
   - `quizen.questions`: PRD 제약을 만족하는 질문 생성 옵션/스텁 + Gemini JSON 스키마 기반 LLM 생성
...
4. 실행 예시
   ...
   from quizen.llm import build_default_llm_client

   lectures = [Lecture(order="001", id="L1", title="샘플 강의")]
   runner = build_default_runner(
       lectures,
       llm_client=build_default_llm_client(),  # GOOGLE_API_KEY 기반 LLM 질문 생성
       question_options=QuestionGenerationOptions(total_questions=4),
   )
   ctx = runner.run()
...
6. Drive → Sheets 파이프라인 한 번에 실행하기
   ...
   - 메타 탭(`quizen_meta`)에는 PART 테이블과 문항 매핑 외에 다음 데이터가 추가로 수집됩니다.
     - 파이프라인 이벤트 로그(`pipeline_event`, payload JSON)
     - PART별 점수 분포(문항 수, 평균/최소/최대 validity score, 기준 미달 건수)
     - 경고 요약(파일명 파싱 실패, PART 분류 fallback 등의 메시지)
     - 외부 호출 결과(LLM/Drive/Sheets, status=success/delayed/error + error_code/message)
   - API/파일 로깅 페이로드(`result["meta_report"]`)에서 동일한 정보를 JSON 형태로 조회할 수 있으며, 실패한 외부 호출은 `result["call_failures"]`를 통해 바로 확인할 수 있습니다.
